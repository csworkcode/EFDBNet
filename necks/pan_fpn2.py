...
class PANFPN(nn.Layer):
    ...
    def forward(self, x):
        ...
        f4_ = self.psa(f4)
        f3_ = f3 + F.upsample(
            f4_, scale_factor=2, mode="nearest", align_mode=1)
        f2_ = f2 + F.upsample(
        ...
        # f2_ = self.smooth_layer2_1(self.dwconv2_1(self._upsample_add(f3, f2)))
        # f1_ = self.smooth_layer1_1(self.dwconv1_1(self._upsample_add(f2, f1)))
