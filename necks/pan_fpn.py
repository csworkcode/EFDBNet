...
class PANFPN(nn.Layer):
    def __init__(self, in_channels, out_channels, use_asf=False, **kwargs):
        ...
        # self.cbam1 = CBAMModule(self.out_channels, 20, 20)
        # self.cbam2 = CBAMModule(self.out_channels, 40, 40)
        # self.cbam3 = CBAMModule(self.out_channels, 80, 80)
        #self.cbam4 = CBAMModule(self.out_channels, 160, 160)

        # self.cbam5 = CBAMModule(self.out_channels, 36, 64)
        # self.cbam6 = CBAMModule(self.out_channels, 72, 128)
        # self.cbam7 = CBAMModule(self.out_channels, 144, 256)
        #self.cbam8 = CBAMModule(self.out_channels, 288, 512)
    ...
    def forward(self, x):
        ...
        f3_ = f3 + F.upsample(
            f4, scale_factor=2, mode="nearest", align_mode=1)
        f2_ = f2 + F.upsample(
            f3_, scale_factor=2, mode="nearest", align_mode=1)
        f1_ = f1 + F.upsample(
            f2_, scale_factor=2, mode="nearest", align_mode=1)
...
class RSEPAN(nn.Layer):
    def __init__(self, in_channels, out_channels, shortcut=True, **kwargs):
        super(RSEPAN, self).__init__()
        self.out_channels = out_channels
    ...
    def forward(self, x):
        ...
        f4 = self.ins_conv[3](c4)  # 调整通道
        f3 = self.ins_conv[2](c3)
        f2 = self.ins_conv[1](c2)
        f1 = self.ins_conv[0](c1)

        f4_ = f4
        f3_ = f3 + F.upsample(
            f4_, scale_factor=2, mode="nearest", align_mode=1)  # 1/16
        f2_ = f2 + F.upsample(
            f3_, scale_factor=2, mode="nearest", align_mode=1)  # 1/8
        f1_ = f1 + F.upsample(
            f2_, scale_factor=2, mode="nearest", align_mode=1)  # 1/4
        ...
        p5 = self.inp_conv[3](f4)
        p4 = self.inp_conv[2](f3)
        p3 = self.inp_conv[1](f2)
        p2 = self.inp_conv[0](f1)
